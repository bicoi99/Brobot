...
// Servo
Servo myServo;
#define HOLD 5 // Hold atom
#define DROP 30 // Drop atom
...
const float d1 = 13.8; // need input
const float d2 = 13.8;
...
bool colour; // purple side is true, yellow is false
...
void purple() {
  ...
  delay(1000);
  movement(fwd, 20, 0);
  delay(1000); // remove this since collecting the outer most greenium is not very hard with current mechanism
  movement(rtn, 0, 25.6 * (M_PI / 180));
  movement(fwd, 55.9, 0);
  movement(rtn, 0, 79.5 * (M_PI / 180));
  movement(fwd, 109.836, 0);
  movement(rtn, 0, 100.5 * (M_PI / 180));
  movement(bwd, 51.5, 0);
  movement(fwd, 199, 0);
  movement(rtn, 0, 90 * (M_PI / 180));
  movement(fwd, 139, 0);
  // right arc 90 degrees 20 radius (need to think about how to collect blue and green at small)
  // forward x (TBD) whole slope is 765.46 mm
}
